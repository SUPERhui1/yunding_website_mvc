...
function fetchnews(pageNum) {
    ...
    $.ajax(url1,
        {
            ...
            success: function (data) {
                ...
                for (i = 0; i < data1.length; i++) {
                    html1 +=
                        `
                        <div class="manageright_centext_item">
                        <p class="manageright_centext_item_p1">新闻Id:${data1[i].newsId}</p>
                        <p  class="manageright_centext_item_p2">新闻标题: <a href="/yunding/news.html?newsId=${data1[i].newsId}">${data1[i].newsTitle}</a></p>
                        <p  class="manageright_centext_item_p3  ">新闻位置: ${data1[i].newsPlace}</p>
                        <button  class="manageright_centext_item_p4" onclick="confirmDeleteNews(${data1[i].newsId},${pageNum})">删除</button>
                        <button  class="manageright_centext_item_p4" onclick="window.location='/ueditor/news.html?id='+${data1[i].newsId}">修改</button>
                        </div>
                `;
                    $('.manageright_centext').html(html1);
    ...
    let html2 = `<button onclick="newsPrePage()">上一页</button>
				<button onclick="newsNextPage()" style="margin-right: 600px;">下一页</button>
...
function news_onload(id) {
    ...
    $.ajax(url, {
        ...
        success: (data) => {
            ...
            $('.bottomdiv').html(' <input type = "button"  value = "修改" onClick = "newupdate(window.location.search.substr(window.location.search.lastIndexOf(\'=\')+1))" class="euploadBtn"/ >   <input type = "button" id = "submit3" value = "返回" onClick = "window.location=\'/H-ui.admin/news-list.html\'" class="euploadBtn"/ >')
        }
    })
}
...
function fetcharticle(pageNum) {
        ...
        success: function (data) {
            ...
            for (i = 0; i < data1.length; i++) {
                ...
                html1 +=
                    `
                        <div class="manageright_centext_item">
                        <p class="manageright_centext_item_p1">文章Id:${data1[i].articleId}</p>
                        <p  class="manageright_centext_item_p2">文章标题: <a href="/yunding/article.html?articleId=${data1[i].articleId}">${data1[i].articleIntroduce}</a></p>
                        <p  class="manageright_centext_item_p3  ">文章类型: ${getArticleCategoryId(data1[i].articleCategoryId)}</p>
                       <button  class="manageright_centext_item_p4" onclick="confirmDeleteArticle(${data1[i].articleId},${pageNum})">删除</button>
                        <button  class="manageright_centext_item_p4" onclick="window.location='/ueditor/article.html?id='+${data1[i].articleId}">修改</button>
                        </div>
                `;
                $('.manageright_centext').html(html1);
                $('#pageIndex').text("第" + pageNum + "页");
    ...
    let html2 = `<button onclick="articlePrePage()">上一页</button>
				<button onclick="articleNextPage()" style="margin-right: 600px;">下一页</button>
...
function article_onload(id) {
    ...
    $.ajax(url, {
        ...
        success: (data) => {
            ...
            $('.bottomdiv').html(' <input type = "button"  value = "修改" onclick = "articleupdate(window.location.search.substr(window.location.search.lastIndexOf(\'=\')+1))" class="euploadBtn"/ >   <input type = "button" id = "submit3" value = "返回" onClick = "window.location=\'/yunding_management/management.html\'" class="euploadBtn"/ >')
        }
    })
}
...
function fetchinform(pageNum) {
        ...
        success: function (data) {
            ...
            for (i = 0; i < data1.length; i++) {
                html1 +=
                    `
                        <div class="manageright_centext_item">
                        <p class="manageright_centext_item_p1">通知Id:${data1[i].informId}</p>
                        <p  class="manageright_centext_item_p2">通知内容:${data1[i].informContent}</p>
                        <button  class="manageright_centext_item_p4" onclick="confirmDeleteInform(${data1[i].informId},${pageNum})">删除</button>
                        </div>
                `;
                $('.manageright_centext').html(html1);
                $('#pageIndex').text("第" + pageNum + "页");
    ...
    let html2 = `<button onclick="informPrePage()">上一页</button>
				<button onclick="informNextPage()" style="margin-right: 600px;">下一页</button>
				<span id="pageIndex">第 页</span><span>/</span><span id="pageSum">第  页</span>`;
    $('.manageright_bottom_items').html(html2);
}
...
function fetchwork(pageNum) {
            ...
            success: function (data) {
                ...
                for (i = 0; i < data1.length; i++) {
                    html1 +=
                        `
                        <div class="manageright_centext_item">
                        <p class="manageright_centext_item_p1"> 作品Id：${data1[i].workId}</p>
                        <p  class="manageright_centext_item_p2">作品名称：${data1[i].workName}</p>
                        <button  class="manageright_centext_item_p4" onclick="confirmDeleteWork(${data1[i].workId},${pageNum})">删除</button>
                        <button  class="manageright_centext_item_p4" onclick="window.location='/ueditor/work.html?id='+${data1[i].workId}">修改</button>
                        </div>
    ...
    let html2 = `<button onclick="workPrePage()">上一页</button>
				<button onclick="workNextPage()" style="margin-right: 600px;">下一页</button>
				<span id="pageIndex">第 页</span><span>/</span><span id="pageSum">第  页</span>`;
    $('.manageright_bottom_items').html(html2);
...
function work_onload(id) {
    ...
    $.ajax(url, {
        ...
        success: (data) => {
            ...
            $('.bottomdiv').html(' <input type = "button"  value = "修改" onclick = "workupdate(window.location.search.substr(window.location.search.lastIndexOf(\'=\')+1))" class="euploadBtn"/ >   <input type = "button" id = "submit3" value = "返回" onClick = "window.location=\'/yunding_management/management.html\'" class="euploadBtn"/ >')
        }
    })
}
...
function fetchmember(pageNum) {
            ...
            success: function (data) {
                ...
                for (i = 0; i < data.data.length; i++) {
                    html1 +=
                        `
                        <div class="manageright_centext_item">
                        <p class="manageright_centext_item_p1">成员Id:${data.data[i].newsId}</p>
                        <p  class="manageright_centext_item_p2">成员姓名: <a href="/yunding/news.html?newsId=${data.data[i].memberId}">${data.data[i].memberName}</a></p>
                        <p  class="manageright_centext_item_p3  ">成员职位: ${data.data[i].memberPosition}</p>
                        <button  class="manageright_centext_item_p4" onclick="confirmDeleteMember(${data.data[i].memberId},${pageNum})">删除</button>
                        <button  class="manageright_centext_item_p4" onclick="window.location='/ueditor/member.html?id='+${data.data[i].memberId}">修改</button>
                        </div>
                `;
                    $('.manageright_centext').html(html1);
                    $('#pageIndex').text("第" + pageNum + "页");
    ...
    let html2 = `<button onclick="memberPrePage()">上一页</button>
				<button onclick="memberNextPage()" style="margin-right: 600px;">下一页</button>
				<span id="pageIndex">第一页</span><span>/</span><span id="pageSum">第  页</span>`;
    $('.manageright_bottom_items').html(html2);
}
...
function member_onload(id) {
    ...
    $.ajax(url, {
        ...
        success: (data) => {
            ...
            $('.bottomdiv').html(' <input type = "button"  value = "修改" onclick = "memberupdate(window.location.search.substr(window.location.search.lastIndexOf(\'=\')+1))" class="euploadBtn"/ >   <input type = "button" id = "submit3" value = "返回" onClick = "window.location=\'/yunding_management/management.html\'" class="euploadBtn"/ >')
        }
    })
}
